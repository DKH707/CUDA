...
void SetUpCudaDevices(int vectorSize)
{
	...
	GridSize.x = GridDimCalcX(N,BLOCK_SIZE); //Makes enough blocks to deal with the whole vector.
...
{					
	...
	//Setting the vector to zero so the extra values will not affect the dot product.
...
__global__ void DotProductGPU(float *a, float *b, float *DotGPU, int n)
{
	...
	__syncthreads();
	for (int i = blockDim.x; i >= 2; i/=2)
	{
		if (threadNumber < i/2 && (vectorNumber + i/2) < n)
			{
				c_sh[threadNumber] += c_sh[threadNumber+i/2];
			}
		__syncthreads();
	}
	//***********************************

...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS*2)
	{
        ...
        cudaMemcpyAsync(C_CPU+i, C0_GPU, DATA_CHUNKS*sizeof(float),cudaMemcpyDeviceToHost, Stream2);
        cudaMemcpyAsync(C_CPU+i+DATA_CHUNKS, C1_GPU, DATA_CHUNKS*sizeof(float),cudaMemcpyDeviceToHost, Stream2);

        //trigAdditionGPU<<<GridSize, BlockSize, 0, Stream2>>>(A1_GPU, B1_GPU,C1_GPU, DATA_CHUNKS);

...
__device__ float getEscapeValue(float x, float y, float a, float b)
{
	...
	while (mag < maxMag && count < maxCount) 
	{
		...
		tempX = x; 
		x = 1/pow(cos(x*x - y*y + a),2);
		y = 1/sin((2.0 * tempX * y) + b);
		mag = sqrt(x*x + y*y);
		count++;
	}
...
__global__ void colorPixels(float *pixels, float xMin, float yMin, float dx, float dy, float a, float b) 
{
	...
	{
		pixels[id]   = 0.5; //Setting the red
		pixels[id+1] = 0.0; //Setting the green
		pixels[id+2] = 0.85; //Setting the blue
	}
	else
	{
...
void adjustSeed()
{
	...
	temp = ((float)rand()/(float)RAND_MAX)*((pow(sin(2*RealSeed),2) * pow(cos(2*DeltaSeed),2))+1.0); // Get random number between -1 at 1.
	RealSeed += DeltaSeed*temp;
	
	temp = ((float)rand()/(float)RAND_MAX)*((sin(ImaginarySeed)) * (sin(RealSeed)*cos(DeltaSeed))) * 2.0; // Get random number between -1 at 1.
